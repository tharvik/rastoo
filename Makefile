help:
	...
	@echo the next targets need FLASH_DEV to be set to the device to use, such as /dev/mmcblk0
	...
	@echo '    $(MAKE) FLASH_DEV=/dev/mmcblk0 flash-init flash-mount chroot'
	...
	@echo '    $(MAKE) FLASH_DEV=/dev/mmcblk0 flash-mount chroot'
...
stage3_subdir := $(firstword $(shell curl -s "$(distfiles_stage3)/latest-stage3-armv6j_hardfp.txt" | tail -n1))
...
/usr/bin/qemu-arm:
	emerge app-emulation/qemu[qemu_user_targets_arm,static-user]
$(root_dir)/usr/bin/qemu-arm: | /usr/bin/qemu-arm $(progress_dir)/portage_extracted
	quickpkg qemu
	ROOT=$(root_dir) emerge --usepkgonly --oneshot --nodeps app-emulation/qemu

/proc/sys/fs/binfmt_misc:
	$(error need a kernel with CONFIG_BINFMT_MISC=y)
$(progress_tmp_dir)/binfmt_registered: | $(progress_tmp_dir) $(root_dir)/usr/bin/qemu-arm /proc/sys/fs/binfmt_misc
...
.PHONY: chroot
chroot: $(root_dir)/etc/resolv.conf $(progress_tmp_dir)/root_subdir_mounted | $(progress_tmp_dir)/binfmt_registered
